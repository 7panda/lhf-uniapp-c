...
const user = defineStore({
  ...
  state: () => ({
    ...
    isLogin: false, // 💡 强制设为 false，彻底关闭登录态
    numData: {
      coupons_num: 0,
      order_num: { aftersale: 0, nocomment: 0, noget: 0, nosend: 0, unpaid: 0 },
    },
    agentInfo: {},
    lastUpdateTime: 0,
    smsUUID: ''
  ...
  actions: {
    // 💡 提供空函数，防止其他页面调用时报错
    async getInfo() { return this.userInfo; },
    async getAgentInfo() { return {}; },
    async getNumData() { },
    setToken(token = '') { return false; },
    async updateUserData() { },
    resetUserData() { },
    async logout() { return true; }
  },
  
  // 💡 依然保留持久化，但不存任何东西，体积最小
  persist: {
    enabled: true,
    strategies: [{ key: 'user-store' }],
  },
});
